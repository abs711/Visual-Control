...
def getDataloader(train_dict,test_dict,val_dict,model_type,model_params, uid_manager):
      ...
      if any(substring in model_type for substring in substring_list):
         ...
         if sanity_check==False:
                 test_loader=torch.utils.data.DataLoader(test_dataset,model_params['batch_size'],num_workers=1, shuffle=False)
                 val_loader=torch.utils.data.DataLoader(val_dataset,batch_size=model_params['batch_size'],num_workers=2, shuffle=False)
         else:
                 test_loader = None
                 val_loader = None
