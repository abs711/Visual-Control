...
model_type_dicts={'history_models': ['HistorySeqToOne','HistorySeqToTwo','HistorySeqToSeq_Ankle','NARX_Ankle'],
                  ...
                  'vision_models':['OpticalFlow4Prosthetics'],
...
def vision_iters(batch,model,phase):
    
    with torch.no_grad():
        batchX,batchUserStat,batchFrames,batchY=batch
        batchFrames=batchFrames.contiguous().to(device)
...
def vision_evaluate(model,phase,model_type,dataloader)  : 
    ...
    for batch_idx,batch in enumerate(dataloader):   
       ...
       if model_type in  model_type_dicts['vision_models']:

             batchY,batchY_pred= vision_iters(batch,model,phase)
...
def vis_evaluate(model,phase,model_type,dataloader) :
    
#       if model_type =='SeqToOne' or model_type=='HistorySeqToOne' or model_type =='TCN_Ankle' or model_type =='NARX_Ankle' or  model_type=='NoHistoryNARX_Ankle':
        print ("Model type", model_type )
        yTests,yPreds = vision_evaluate(model,phase,model_type,dataloader)
