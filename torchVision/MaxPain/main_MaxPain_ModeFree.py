...
if __name__ == "__main__":
    ...
    param_vals = { 'num_epochs': [2000], 'batch_size':[100,100000], 'minibatch_GD': True, 'num_trials':[3],
                  
                   'seq_length':[10],'target_len':[1], 'pred_horizon':[30], # THESE ARE FIXED IN THE DATAMANAGER. NO LIST.
                   
                   'num_layers':[2],'num_hid_units':(32), 'num_features':(57), 'num_vis_features':[3],'num_classes': [8], 'weighted_classification': [True],
                   'num_fc_layers': [4],'fc_taper':[2],
                   'pretrained_LSTM': False, 'backprop_kinematics': True, 'backprop_vision': False, 'zero_like_vision':False,

                   'window_step':[1],'sub_sample_factor':[1], 'frame_subsample_factor':[0], # THESE ARE FIXED IN THE DATAMANAGER. NO LIST.

                   'noise_std':[0.02],'clip':[-1],'dropout':[0.0], 'pretrainC2A_w_Dropout':True, 'regularization':[0],
                   'learning_rate': [1e-3,1e-5], 'custom_lr_decay_rate':[0.1], 'custom_lr_decay_step':[90],
                   'use_scheduler': [True], 'plateau_schedular_patience':10, 'decay_factor': 0.1,'EarlyStoppingPatience':50, 'Early_Stopping_delta':0,'valid_patience':1,
                   'num_workers':(4),'pin_memory':False,'non_blocking':True, 'device':(0)}
    
#    data_dir = os.path.dirname(__file__) + '/data/vision_kinematics/'
#    config_file_name = "ConfigFiles/v6_cf_flat_wstop(Ankle).csv"
    
    model_type='SeqToTwo'##'Scene4Prosthetics'##
    runner.SetTrainingOpts(sanityflag=False,use_single_batch=False,overlap_datatransfers=param_vals['non_blocking'],model_name=model_type,indie_checkpoints=True)#,param_vals['backprop_vision'],clipGradients,indie_checkpoints,vision_substrings,numscenesamples=paramvals['numscenesample'])
    scheduler_type = 'ReduceOnPlateau'
    folder_annotation='ModeFree_Unstructuredinc_30ph_full2'#''ModeFree_Test_cycle'#'ModeFree_Unstructured'#'Scene_Pretraining_sanity'#


    data_dir = '/home/spc/pytorch_challenge-master/data/vision_kinematics/Unstructured_data/Unstructured_Data/' #"/home/spc/Mega-Tron/SeqtoSeq_Kinematics/"#'/home/spc/pytorch_challenge-master/data/vision_kinematics/Unstructured_data/Unstructured_Data/' ##"D:/Vision_Replication/data/vision_pretraining/" #
    config_file_name ="v6_cf_unstruct_scene-full2exclude.csv" #v6_cf_flat(Knee+Ankle).csv"#v6_cf_unstruct_scene-full.csv" # " "ConfigFiles/v6_cf_unstruct_pretraining.csv"
